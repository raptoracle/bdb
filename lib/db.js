...
class DB {
  ...
  /** @type {LevelDBClass|MemDBClass|MongoDBClass} */
  backend;
  ...
  /** @type {LevelDB|MemDB|MongoDB} */
  binding;

  /**
   * Create a DB instance.
   * @constructor
   * @param {MemDBClass|LevelDBClass|MongoDBClass} backend - Database backend.
  ...
  init() {
    ...
    if (Backend.leveldown) {
      ...
      this.binding = new Backend(this.options);
      this.mongo = true;
    } else {
      // @ts-ignore
      this.binding = new Backend(this.location);
